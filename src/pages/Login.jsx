...
const Login = () => {
  ...
  React.useEffect(() => {
    ...
    if (params.get('status') === 'signup_success') {
      setSuccessMessage('تم إنشاء حسابك بنجاح! الرجاء تسجيل الدخول.');
    }
  }, [location.search]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.user) {
        // The auth state change will be handled by the global store listener
        // Just navigate to the home page or a protected route like profile.
        navigate('/profile');
      }
    ...
    <div className="bg-zinc-950 text-white min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md mx-auto bg-black border border-violet-500/30 rounded-3xl shadow-lg shadow-violet-500/10 p-8">
        <div className="text-center mb-8">
          ...
          <h2 className="text-4xl font-extrabold tracking-tight">تسجيل الدخول</h2>
          <p className="mt-3 text-zinc-400">مرحباً بعودتك! أدخل بياناتك للمتابعة.</p>
        </div>
        
        <form onSubmit={handleSubmit} className="space-y-6">
          {error && <div className="bg-red-900/50 border border-red-500 text-red-200 p-3 rounded-lg text-center">{error}</div>}
          {successMessage && <div className="bg-green-900/50 border border-green-500 text-green-200 p-3 rounded-lg text-center">{successMessage}</div>}
