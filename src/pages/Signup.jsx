...
const Signup = () => {
    ...
    const handleSignup = async (e) => {
        ...
        try {
            ...
            const { data, error: signUpError } = await window.__SUPABASE__.auth.signUp({
                email,
                password,
                options: {
                    data: { full_name: name }
            ...
            // If email confirmation is required, Supabase handles it. Otherwise user is logged in.
            navigate('/profile');
        } catch (err) {
    ...
    return (
        <div className="min-h-screen bg-black pt-28 pb-20 px-4 flex items-center justify-center">
            <div className="max-w-md w-full bg-zinc-950 p-8 rounded-3xl border border-white/10 shadow-2xl">
                ...
                <p className="text-gray-400 mb-8">انضم إلينا وابدأ تجربة تسوق فريدة.</p>
